...
public final class InspectPlayers {
    ...
    public static boolean hasBlock(OfflinePlayer pl){
        return inspectModePlayers.containsKey(pl.getUniqueId()) && 
               inspectModePlayers.get(pl.getUniqueId()) != null;
    }
    ...
    public static long getTimeLeft(OfflinePlayer player) {
        if (!isCooldown(player)) {
            return 0;
        }
        return Math.max(0, cooldownPlayers.get(player.getUniqueId()) - System.currentTimeMillis());
    }
